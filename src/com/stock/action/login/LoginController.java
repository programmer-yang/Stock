...
public class LoginController {
	...
	private void holdCurrentUser(Map<String,Object> model,HttpSession session)
	{
		try {
			...
			if(null == session.getAttribute("cuurentUserName"))
			{
				...
				if(principal instanceof UserDetails){
					...
					while(it.hasNext()){  
						authoritys.append(((GrantedAuthority)it.next()).getAuthority()+",");  
					}
					currentUser.setAuthority(authoritys.toString());
				}
				session.setAttribute("cuurentUserName", currentUser.getUsername());
				
				/**
				 * 获取权限后如何处理，在这里是实现即可
				 */
//				model.put("currentUser", currentUser);
				log.info("CurrentUser:"+currentUser.getUsername()+":"+currentUser.getAuthority());
			}
		} catch (Exception e) {
